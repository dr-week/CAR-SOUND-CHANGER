...
export function createVehicleState(profile: VehicleProfile): VehicleState { return { profile, rpm: profile.idleRpm, gear: 1, throttle: 0, brake: 0, speedKph: 0, gpsSpeedKph: null }; }
export function stepVehicle(state: VehicleState, dt: number): void {
  ...
  const targetRpm = Math.max(state.profile.idleRpm, Math.max(0, activeSpeed) * 29 * (GEAR_RATIOS[state.gear] ?? 1) + state.throttle * 900);
