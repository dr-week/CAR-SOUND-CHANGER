...
export function createGreenScore(): GreenScore {
...
export function updateGreenScore(score: GreenScore, vehicle: VehicleState, dt: number): void {
  ...
  if (!harshBrake && !highRpm && !harshThrottle && vehicle.throttle > 0 && vehicle.brake === 0)
    score.earned += 0.15 * dt;
  score.points = Math.round(
    Math.max(0, Math.min(100, 100 + score.earned - Object.values(score.penalties).reduce((a, b) => a + b, 0))),
  );
  score.previousThrottle = vehicle.throttle;
}
export function getGreenScoreSummary(score: GreenScore): string {
  return `Brake ${score.penalties.harshBrake.toFixed(1)} · RPM ${score.penalties.highRpm.toFixed(1)} · throttle ${score.penalties.harshThrottle.toFixed(1)} penalty points`;
}
