...
| :--- | :--- | :--- |
| Dedicated full-screen performance cluster was absent from primary rail navigation. | Drivers seeking an immersive cockpit gauge mode had to rely solely on the Home HUD ribbon. | Added dedicated `Gauges` mode to the Navigation Rail activating the full-screen `InstrumentDashboard.vue` twin cluster. |
| All Apps drawer duplicated destinations already pinned on the Rail (Navigation, Settings, Sound Lab). | Increased visual scanning time and cluttered the application drawer with redundant icons. | De-duplicated All Apps to focus exclusively on secondary automotive utilities (Vehicle Diagnostics, FM Tuner, Parking Camera, Pairing). |
| Dead CSS classes (`.drive-card`, `.climate-card`, `.road`, `.touch-pedal`) remained in stylesheet. | Cluttered the stylesheet and risked conflicting with active avant-garde styling rules. | Purged ~150 lines of unused legacy CSS classes from `app.css`. |
| Speaker zone selector used a rudimentary 4-dot diagram. | Failed to convey the spatial acoustic soundstage of the synthesized engine audio. | Replaced with an interactive SVG automotive acoustic blueprint featuring dynamic sound propagation waves. |
